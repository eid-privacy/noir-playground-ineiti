...
def format_for_noir(keys):
    """Format keys for use in Noir circuit."""
    print("\n=== Issuer Keys (Noir Format) ===")
    print(f"Private Key: 0x{keys['private_key']}")
    print(f"Public Key X: 0x{keys['public_key_x']}")
    print(f"Public Key Y: 0x{keys['public_key_y']}")

    # Format as byte arrays for Noir
    x_bytes = [f"0x{keys['public_key_x'][i:i+2]}" for i in range(0, 64, 2)]
    y_bytes = [f"0x{keys['public_key_y'][i:i+2]}" for i in range(0, 64, 2)]

    print(f"\nPublic Key X (bytes): [{', '.join(x_bytes)}]")
    print(f"Public Key Y (bytes): [{', '.join(y_bytes)}]")

if __name__ == "__main__":
    print("Generating secp256k1 issuer key pair...")

    keys = generate_issuer_keys()
    save_keys_to_file(keys, 'scripts/issuer_keys.json')
    format_for_noir(keys)

    print("\n✅ Issuer keys generated successfully!")
    print("Next: Use sign_credential.py to sign a credential with these keys.")